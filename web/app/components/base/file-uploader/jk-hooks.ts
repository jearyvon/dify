...
const isPublicWebAppUpload = (params: ReturnType<typeof useParams>) => {
  return !!(params.token ?? params.appid)
}

// eslint-disable-next-line react/no-unnecessary-use-prefix -- keep API parity with hooks.ts
...
export const useFile = (fileConfig: FileUpload, noNeedToCheckEnable = true) => {
  ...
  const handleLoadFileFromLink = useCallback((url: string) => {
    ...
    uploadRemoteFileInfo(url, isPublicWebAppUpload(params)).then((res) => {
      const newFile = {
        ...uploadingFile,
        type: res.mime_type,
        size: res.size,
  ...
  }, [checkSizeLimit, handleAddFile, handleUpdateFile, t, handleRemoveFile, fileConfig?.allowed_file_types, fileConfig.allowed_file_extensions, startProgressTimer, params])
  ...
  }, [noNeedToCheckEnable, checkSizeLimit, t, handleAddFile, handleUpdateFile, params, fileConfig?.allowed_file_types, fileConfig?.allowed_file_extensions, fileConfig?.enabled])
